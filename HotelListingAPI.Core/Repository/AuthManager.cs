...
namespace HotelListingAPI.Core.Repository
{
    public class AuthManager : IAuthManager
    {
        ...
        public async Task<IEnumerable<IdentityError>> Register(ApiUserDto userDto)
        {
            ...
            var results = await _userManager.CreateAsync(_user, userDto.Password);

            if (results.Succeeded)
            {
                await _userManager.AddToRoleAsync(_user, "User"); // only assigns users not admins
        ...
        public async Task<AuthResponseDto> VerifyRefreshTokens(AuthResponseDto request)
        {
            ...
            // Read the token
            var tokenContent = jwtSecurityTokenHandeler.ReadJwtToken(request.Token);
            // Get the claims
